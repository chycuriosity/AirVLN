...
当前路口 judge 的提示词也做了收紧：

- 优先把“有明确道路/街角/桥头入口/走廊口几何结构”的画面判为 `true`
- 优先把“广场式开阔区域、岸边/水边开口、步道缓弯、屋顶或空地开口”判为 `false`
- 同时保留“街角 + 斑马线 + 建筑转角”这类真正街道路口作为 `true`

这能减少把弱分叉、庭院步道、开阔空地误记为实验目标难点的情况。
